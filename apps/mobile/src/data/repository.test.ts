...
function query(table: string) {
  ...
  const chain: any = { eq: () => chain, in: () => chain, order: () => chain, limit: () => chain, maybeSingle: async () => ({ data: result.data[0] ?? null, error: null }), then: (resolve: (value: unknown) => unknown) => resolve(result) };
  return chain;
}
(mockSupabase.from as any).mockImplementation((table: string) => ({ select: () => query(table), update: () => query(table), upsert: () => query(table) }));

import { getAssignments, getAttendanceSummaries, getCurrentProfile, getTimetable, saveSubmissionDraft, submitAssignment } from './index';
...
beforeEach(() => { Object.keys(mockRows).forEach(key => delete mockRows[key]); (mockSupabase.auth.getSession as any).mockResolvedValue({ data: { session: { user: { id: 'student-a' } } } }); });
...
test('scopes assignments to active enrollments and derives exact status', async () => {
  ...
  mockRows.assignments = [{ id: 'assignment-a', course_id: 'course-a', courses: [{ title: 'Networks' }], title: 'Lab', instructions: '', max_score: 10 }];
  mockRows.submissions = [{ assignment_id: 'assignment-a', status: 'submitted' }];
