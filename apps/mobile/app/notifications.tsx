...
export default function NotificationsScreen() {
  ...
  const markRead = async (id: string) => { try { await repository.markNotificationRead(id); await queryClient.invalidateQueries({ queryKey: queryKeys.notifications }); } catch (error) { Alert.alert('Could not mark notification read', error instanceof Error ? error.message : 'The notification remains unread.'); } };
